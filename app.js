...
const response = await fetch('./scene.cvc')
...
const player = Module.Cavalry.MakeWithPath('scene.cvc')
...
// How often to tell the parent page (Framer) the real playhead position
// and play/pause state. A few times a second is plenty for a smooth-
// looking timeline without flooding postMessage.
const TIME_REPORT_INTERVAL_MS = 100
let lastTimeReportedAt = 0

function reportPlaybackTime(timestamp) {
    if (timestamp - lastTimeReportedAt < TIME_REPORT_INTERVAL_MS) return
    lastTimeReportedAt = timestamp
    ...
    const duration = fps > 0 ? (endFrame - startFrame + 1) / fps : 0
    const current = fps > 0 ? (currentFrame - startFrame) / fps : 0
